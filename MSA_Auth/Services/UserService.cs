...
namespace MSA_Auth_API.Services
{
    public class UserService : IAccountService
    {
        ...
        public UserService(IAccountRepository userRepository, IOptions<AuthenticationSettings> authenticationSettings)
        {
            _accountRepository = userRepository;
            _authenticationSettings = authenticationSettings.Value;
        }

        public async Task<AccountResponse> GetUserAsync(GetAccountRequest request, CancellationToken cancellationToken)
        {
        ...
        public async Task<AccountResponse> SignUpAsync(SignUpRequest request, CancellationToken cancellationToken)
        {
            var account = new Models.Account { Email = request.Email};
            bool isCreated = await _accountRepository.SignUpAsync(account, request.Password, cancellationToken);
